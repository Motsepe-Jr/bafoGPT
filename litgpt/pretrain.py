...
def setup(
    ...
    train: TrainArgs = TrainArgs(
        ...
        global_batch_size=4,
        micro_batch_size=1,
        max_tokens=int(78555254),  # 200 m tokens
        max_norm=1.0,
        min_lr=4e-5,
    ...
    print("Model Config Expanded Vocab Size",  model_config.vocab_size)
    assert model_config.vocab_size == 288256
...
def main(
    ...
    log_gpu_memory("Before model instantiation")
    ...
    log_gpu_memory("Before data loading")
    train_dataloader, val_dataloader = get_dataloaders(fabric, data, tokenizer, train, model.max_seq_length)
    train_dataloader, val_dataloader = fabric.setup_dataloaders(train_dataloader, val_dataloader)
    log_gpu_memory("After data loading")


    log_gpu_memory("Before loading initial checkpoint")
    if initial_checkpoint_dir:
        fabric.load_raw(initial_checkpoint_dir / "lit_model.pth", model)
    log_gpu_memory("After loading initial checkpoint")


    print("read model: ", initial_checkpoint_dir / "lit_model.pth")
...
def fit(
    ...
    if eval.initial_validation:
        ...
        val_loss = "n/a"
        fabric.print("Verifying settings completed")

    throughput = ThroughputMonitor(fabric, window_size=5)
   
    log_gpu_memory("Before Torch Meta")
    with torch.device("meta"):
        ...
        del meta_model, x
    log_gpu_memory("After Torch Meta")
 
    log_gpu_memory("Before tokens calcs and CycleIterator")
    ...
    log_gpu_memory("After tokens calcs and CycleIterator")
    print(max_iters, tokens_per_iter, initial_iter)
