...
## Finetuning 🤗 [Hub] (https://huggingface.co/ChallengerSpaceShuttle/finetuned-qlora-bafoGPT-2/tree/main)

I finetuned the base model **BafoGPT-2-2B-base** with [QLoRA](https://arxiv.org/abs/2305.14314) an efficient finetuning approach that reduces memory usage by  backpropagating gradients through a frozen, 4-bit quantized pretrained language model into Low Rank Adapters~(LoRA) on a **single A10 (24GB) GPU**. Used IsiZulu Wikihow, combiend with translated Alpaca dataset (52K instruction-following data generated by the techniques in the [Self-Instruct](https://arxiv.org/abs/2212.10560)), totally to 200k Instruction Dataset. Due to budget cost we ommitted the IsiZuLu Cosmopedia 100k dataset in our finetuning stage. 

> LoRA was applied to `query`, `key`, `value`, and `projection` matrices, Due compute budget we ommited the `mlp`, The effectiveness of applying LoRA to all linear transformer blocks is verified in QLoRA papers. The `embedding layer` and classification `head layer` were ommited, since we continued to pretrain the Gemma Model on Isizulu dataset, the new tokens consist of  a representation. Common pattern I see, Is that if you finetune model with new tokens, then apply lora to the embedding layer to learn new token  represetation, in this case all the token have been learned in the continued pretraining stage. 

> finetuning used 4-bit quantization (NormalFloat data type) as introduced in the Qlora paper, and added double-quantization
...
To bring the memory consumption down, we used `bfloat16` `--precision "bf16-true"` which is much more efficient compared to `--precision "32-full"`. BF16 (bfloat16) has the same exponent as FP32, which avoids the overflow issues that FP16 often faces. With FP16, which has a limited numerical range (up to 64k), multiplying larger numbers can cause overflow. For example, 250 * 250 = 62500 is fine, but 255 * 255 = 65025 causes an overflow, leading to problems during model training. This forces weights to stay very small. While loss scaling can help, FP16’s limited range becomes an issue with larger models.

BF16 solves this by allowing much larger numbers, like 10,000 * 10,000 = 100 million, without overflow. However, BF16 comes with reduced precision compared to FP16. But in training, where stochastic gradient descent involves gradual corrections, this lower precision isn’t a major problem since the model adjusts in future steps. hance we trained with `blfoat16`


## Merging LoRA Weights (Optional)

Finetuning a model with LoRA generates a `lit_model.pth.lora` file. This file exclusively contains the LoRA weights, which are much smaller than the original model checkpoint to conserve storage space. The weights can be found on the [Hub] (https://huggingface.co/ChallengerSpaceShuttle/finetuned-qlora-bafoGPT-2/tree/main)
